...
const Profilecard = () => {
    ...
    if (!user) {
        return (
            <div className="max-w-xs rounded-xl overflow-hidden border border-gray-200 bg-white p-5">
                <p className="text-center text-gray-500 text-sm">Loading user...</p>
            </div>
        );
    }
        ...
        <div>
            <div className="max-w-xs rounded-xl overflow-hidden border border-gray-200 bg-white">
                ...
                <div className="p-5 pt-8">
                    <h2 className="text-lg font-semibold text-gray-800">{user?.name || "Not Available"}</h2>
                    <p className="text-sm text-gray-500 mb-2">{user?.username || "No Username"}</p>
                    <p className="text-sm text-gray-600 leading-snug">{user?.bio || "No Bio"}</p>
                </div>
