...
const ChatApp = () => {
  ...
  const handleChatClick = async (chat) => {
    ...
    setScreen("chat");
    setShowMediaOptions(false); // Close drawer when entering chat
    ...
    if (chat.isGroup || groupId) {
      ...
      dispatch(setCurrentChatId(groupId || chat._id));
      ...
      // Mark as read
      socket.readChats(
        { chatId: chat._id, unreadCount: chat.unreadCount },
        (response) => {
  ...
  if (screen === "chat") {
    return (
      <div className="fixed bottom-6 right-6 w-96 bg-white rounded-[12px] shadow-2xl overflow-hidden border border-gray-200 z-40 flex flex-col h-96">
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200">
          <div className="flex items-center gap-3">
              ...
              onClick={() => {
                if (selectedChat?.isGroup) {
                  socket.leaveGroupRoom(
                    ...
                    (response) => {
                      console.log("Left group room:", response);
                    }
                  );
                } else {
                  socket.leaveChats(
                ...
                setScreen("list");
                setShowMediaOptions(false); // Close drawer when leaving chat
              }}
            ...
            <img
              ...
              alt={
                selectedChat?.name || selectedChat?.receiverInfo?.name || "Chat"
              }
              className="w-10 h-10 rounded-full object-cover"
            />
        ...
        <div
          className={`absolute bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 transition-transform duration-300 ${
            showMediaOptions
