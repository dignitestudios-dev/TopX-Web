...
import { sendPhoneOTP, verifyPhoneOTP } from "../../redux/slices/onboarding.slice";
...
export default function VerifyAccount({ email, phone, handleNext, handlePrevious }) {
  ...
  const handleVerifyOTP = async (code) => {
    const res = await dispatch(verifyPhoneOTP(code));
    ...
    <div className="bg-white flex items-center justify-center rounded-[19px] w-full p-6 ">
      ...
      <div className="flex flex-col w-full items-center justify-center gap-4 lg:gap-8">
        ...
        <Button
          ...
          size="full"
          loading={isLoading}
          disabled={!user?.isPhoneVerified}   // 🔥 disable if not verified
          className={`w-full flex items-center justify-center ${!user?.isPhoneVerified ? "opacity-60 cursor-not-allowed" : ""
            }`}
        >
