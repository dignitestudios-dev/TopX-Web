      ...
      <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-2xl w-full max-w-lg p-6">
          ...
          {existingMedia.length > 0 && (
            <div className="mb-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                {existingMedia.map((m) => (
                  <div key={m._id} className="relative w-full overflow-hidden rounded-lg border">
                    {m.type === "image" ? (
                      <img src={m.fileUrl} alt="" className="w-full h-32 object-cover" />
                    ) : (
                      <video src={m.fileUrl} controls className="w-full h-32 object-cover" />
                    )}
                    <button
          ...
          {newPreviews.length > 0 && (
            <div className="mb-4">
              <p className="text-sm font-medium text-gray-700 mb-2">New Media (Selected)</p>
              <div className="grid grid-cols-2 gap-3">
                {newPreviews.map((preview, index) => (
                  <div key={index} className="relative w-full overflow-hidden rounded-lg border bg-gray-50">
                    {preview.type === "image" ? (
                      <img src={preview.preview} alt="" className="w-full h-32 object-cover" />
                    ) : (
                      <video src={preview.preview} controls className="w-full h-32 object-cover" />
                    )}
                    <button
          ...
          >
            {newPreviews.length > 0 ? "+ Add More Media" : "+ Add Media (Optional)"}
          </button>
