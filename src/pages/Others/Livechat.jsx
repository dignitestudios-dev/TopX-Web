  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      const presignedRes = await axios.post(
        "https://api.my-topx.com/uploads/presigned-urls",
        {
          files: [
            {
              fileName: file.name,
      ...
      // ✅ CORRECT KEY
      const uploadUrl = presignedRes.data?.data?.uploadUrls?.[0];
      const fileUrl = presignedRes.data?.data?.fileUrls?.[0];
          ...
          <div className="absolute bottom-24 left-8 right-8 bg-white border rounded-2xl shadow-2xl z-20 p-4 max-h-96 flex flex-col">
            <div className="flex justify-between items-center mb-4">
              <input
                ...
                className="bg-gray-100 border-none rounded-lg px-4 py-2 flex-1 text-sm"
                value={gifQuery}
